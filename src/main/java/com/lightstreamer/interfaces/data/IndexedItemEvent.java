...
public interface IndexedItemEvent {
    ...
    /**
     * Returns the maximum index for the fields in the event. The event cannot
     * be empty, so the maximum Index must always exist.
     *
    ...
    /**
     * Returns the index of a named Field. Returns -1 if such a field is not
     * reported in this event.
     * Lightstreamer Kernel will call this method up to once for every distinct
     * client request of the Item. So, the implementation must be very fast.
     *
     * @param  name  A Field name.
     * @return  a 0-based index for the field or -1. The index must not be
     * greater than the maximum index returned by getMaximumIndex().
     */
    int getIndex(@Nonnull String name);

    /**
     * Returns the name of a Field whose index is supplied. Returns null if
     * the Field is not reported in this event.
     * If the Item to which this ItemEvent refers has been subscribed
     * by setting the needsIterator flag as false, the method can always
     * return a null value.
     *
     ...
     * value too). Returns null if the Field is not reported in the ItemEvent.
